...
export const ContentMusicItemRow = React.memo(
  ({ contentMusicItem }: ContentMusicItemRowProps) => {
    const {
      id,
      title,
      position,
      ...
      useMemo(
        () => ({
          extraValidation: (e, item, copy) => {
            if (item.content_list !== contentMusicItem.content_list)
              return false;
    ...
    const handleToggleParsed = () => {
      dispatch(
        updateContentMusicItem({ ...contentMusicItem, parsed: !parsed })
      );
    };
    ...
    const renderTypeSVG = () => {
      if (isPlaylist) {
        return (
          <SVGListBullet className='w-5 text-active-2/80' tooltip={type} />
        );
      }
      if (isSingle) {
